...
import { updateEnvironmentroutesFilterAction } from 'src/app/stores/actions';
import {
...
export class RoutesMenuComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.routesFilterSubscription = this.routesFilter.valueChanges.pipe(
      distinctUntilChanged(),
      debounceTime(50),
      startWith(''),
      tap(search => this.store.update(updateEnvironmentroutesFilterAction(search)))
    ).subscribe();
  }
